    ...
    const getExpirationDate = (car) => {
        ...
        return expirationDate ? (
            <>
                ...
                <span className="text-gray-400 ml-1">({countRemainingDays(expirationDate)} zile)</span>
            </>
        ) : '-';
    };
    ...
    const sendNotification = (car) => {
        if (!disableButton(car.lastNotificationDate)) {
            sendSms(
                ...
                getExpirationDate(car),
                countRemainingDays(getExpirationDate(car))
            );
        }
    };
                    ...
                    <div className="mb-2">
                        ...
                        >
                            {TYPE_OPTIONS.map(option => (
                                <option key={option.value} value={option.value} className="max-sm:text-xs">{option.label}</option>
                            ))}
                        </select>
                    </div>
                    <div className="mb-2">
                    ...
                    {dueCars.length ? (
                        <div className="rounded-lg">
                            <h2 className="duration-300 text-lg md:text-xl font-semibold my-2 md:my-4 text-center">
                                {selectedDuration && selectedType && selectedDuration !== 'expired' ? (
                                    <>
                                        <span className="text-red-600 font-bold">{renderTypeText(selectedType)}</span> următoarelor <span className="text-red-600 font-bold">{dueCars.length}</span> mașini expiră în <span className="text-red-600 font-bold">{renderDurationText(selectedDuration)}!</span>
                                    </>
                                ) : (
                                    <>
                                        Următoarele <span className="text-red-600 font-bold">{dueCars.length}</span> mașini au <span className="text-red-600 font-bold">{renderTypeText(selectedType)}</span> {renderTypeText(selectedType) === 'Rovinieta' ? 'expirată' : 'expirat'}!
                                    </>
                                )}
                            </h2>
                        ...
                        showError && selectedDuration && selectedType && (
                            <div className="flex text-center justify-center text-lg text-red-500 font-bold max-sm:p-2">
                                <h1>
                                    {selectedDuration === 'expired' ? (
                                        renderTypeText(selectedType) === 'ITP-ul' ? "Niciun ITP expirat!" :
                                            renderTypeText(selectedType) === 'Rovinieta' ? "Nicio Rovinietă expirată!" :
                                                renderTypeText(selectedType) === 'RCA-ul' ? "Niciun RCA expirat!" : ''
                                    ) : (
                                        <>
                                            {selectedDuration === 'today' ? '' : 'În următoarele '}
                                            {renderDurationText(selectedDuration)} nu expiră
                                            {renderTypeText(selectedType) === 'ITP-ul' ? " niciun ITP!" :
                                                renderTypeText(selectedType) === 'Rovinieta' ? " nicio Rovinietă!" :
                                                    renderTypeText(selectedType) === 'RCA-ul' ? " niciun RCA!" : ''}
                                        </>
                                    )}
                                </h1>
                            </div>
